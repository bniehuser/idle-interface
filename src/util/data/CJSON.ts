...
const CJSON = (function() {
  ...
  class Node {
    parent: Node|null;
    key: number|string;
    children: {[T in string|number]: Node};
    templateIndex: number|null;
    links: any[];

    constructor(parent: Node|null, key: number|string) {
      this.parent = parent;
      this.key = key;
      this.children = {};
    ...
    public follow( key: number|string ): Node {
  ...
  // key tree.
  function process( root: Node, value: any ): any {
    let result: any[]|{[k: string]: any[]};
    let key;
    let node: Node;
  ...
  function createTemplates( root: Node ) {
    const templates = [];
    const queue: Node[] = [];
    let node: Node;
    let template;
    ...
    while ( queue.length > 0 ) {
      ...
      if ( numChildren > 1 || node.links.length > 0 ) {
        ...
        // id is reached.
        while ( cur && cur.templateIndex === null ) {
          template.unshift( cur.key );
          cur = cur.parent;
        }

        if (cur) { template.unshift( cur.templateIndex ); }
  ...
  function Compress( value: any ): string {

    const root = new Node( null, '' );
    const values = process( root, value );
    const templates = createTemplates( root );

    if ( templates.length > 0 ) {
      return JSON.stringify({ f: 'cjson', t: templates, v: values }, null, undefined);
    } else {
  ...
  function getKeys( templates: any[], indexNum: number ) {
    let keys: number[] = [];
    let index = indexNum;
  ...
  function expand( templates: any[], value: any ): any {
    let result: any;
    let keys = [];
