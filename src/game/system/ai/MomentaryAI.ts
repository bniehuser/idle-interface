...
const runMomentary = (entities: GameEntities) => new Promise((resolve) => {
    ...
    if (!running) {
      ...
      Object.values(entities.gameState.people).forEach(p => {
        if (Math.random() > .999996) {
          entities.gameDispatch({type: 'notify', key: 'money', content: `${p.avatar}${p.name.given} ${p.name.family} found $100 on the ground!`});
        }
      });
      running = false;
    } else {
